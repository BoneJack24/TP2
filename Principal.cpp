	...
	while (nb != 0)
	{
		...
		try
		{
			switch (nb)
			{
				...
				{
                    std::cout<<"principal : L'usager vient d'entrer 1." << std::endl;
					EntreeFichier.open("ReseauAerien.txt", ios::in);
                    std::cout<<"principal : Le programme vient d'ouvrir le fichier 'ReseauAerien.txt'." << std::endl;
                    std::cout<<"principal : Le fichier 'ReseauAerien.txt' est ouvert? " << EntreeFichier.is_open() << std::endl;
					if(EntreeFichier.is_open())
					{
                        std::cout<<"principal : Le programme va charger le réseau du fichier ReseauAerien.txt" << std::endl;
